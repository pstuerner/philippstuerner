...
function chart3(dataRaw) {
    ...
    function predict() {
        let degreeIndex = +degreeSlider.value() - 1,
            xPolyTrain = math.matrix(
                dataRaw.polynomials[degreeIndex].X_poly_train
            ),
            xPolyTest = math.matrix(
                dataRaw.polynomials[degreeIndex].X_poly_test
            );
        ...
        r2Train =
            1 -
            math.sum(squaredResidualsTrain) /
                math.sum(math.square(math.subtract(yTrain, yTrainMean)));
        ...
        r2Test =
            1 -
            math.sum(squaredResidualsTest) /
                math.sum(math.square(math.subtract(yTest, yTestMean)));
        ...
        let lineDataTrain = [
                {
                    v: yHatTrain.valueOf().map(function (d, i) {
                        return {
                            x: xTrain.subset(math.index(1, i)),
                            y: _.clamp(
                                d,
                                yExtentTrain[0] -
                                    math.abs(yExtentTrain[0] * 0.5),
                                yExtentTrain[1] +
                                    math.abs(yExtentTrain[0] * 0.5)
                            ),
                        };
            ...
            lineDataTest = [
                {
                    v: yHatTest.valueOf().map(function (d, i) {
                        return {
                            x: xTest.subset(math.index(1, i)),
                            y: _.clamp(
                                d,
                                yExtentTest[0] - math.abs(yExtentTest[0] * 0.5),
                                yExtentTest[1] + math.abs(yExtentTest[0] * 0.5)
                            ),
                        };
                    }),
                },
     ...
     */
    function dataInit() {
        dataTrain = _.sortBy(dataRaw.data_train, [
            function (o) {
                return o.X;
            },
        ]);
        dataTest = _.sortBy(dataRaw.data_test, [
            function (o) {
                return o.X;
            },
        ]);
    ...
    let degreeSlider = d3
        ...
        noiseSlider = d3
            ...
            .on("end.slider", (val) => {
                d3.json(
                    `https://api.philippstuerner.com/data/poly_data?degrees=20&noise=${+val}`
    ...
    svgDegreeSlider
        .transition()
        .duration(500)
        .call(degreeSlider.min(1).max(20).value(1))
        .attr("id", "chart3-degree-slider");

    svgNoiseSlider
        .transition()
        .duration(500)
        .call(noiseSlider.min(0).max(20).value(4))
...
d3.json(
    "https://api.philippstuerner.com/data/poly_data?degrees=20&noise=5"
