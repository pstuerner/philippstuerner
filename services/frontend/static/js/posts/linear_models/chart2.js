...
function chart2(dataRaw) {
  function predict () {
    ...
    r2 = 1 - (math.sum(squaredResiduals)/math.sum(math.square(math.subtract(y,yMean))))
  }

  function updateMathjax () {
    const nodes = [
      document.getElementById('chart2-mse'),
      document.getElementById('chart2-rmse'),
      document.getElementById('chart2-mae'),
      document.getElementById('chart2-r2'),
    ];
  ...
  function updateMathjaxNewData () {
    let X_ = math.transpose(X),
        matX_ = latexifyMatrix(math.round(X_,2),5,3,5,false,false),
        y_ = math.transpose(y),
        maty_ = latexifyMatrix(math.round(y_,2),5,1,5,false,false),
        XT = math.transpose(X_),
        matXT = latexifyMatrix(math.round(XT,2),3,5,false,5,false),
        XTX = math.multiply(XT,X_),
        matXTX = latexifyMatrix(math.round(XTX,2),3,3,false,false,false),
        XTXinv = math.inv(XTX),
    ...
    const nodes = [
      ...
      document.getElementById('chart2-theta2'),
...
>>> X=np.array([${[...math.round(X._data[1].slice(0,5),2),...['...']].toString()}])
>>> y=np.array([${[...math.round(y._data[0].slice(0,5),2),...['...']].toString()}])
...
>>> theta_hat
array([${theta0_best.toFixed(2)}, ${theta1_best.toFixed(2)}, ${theta2_best.toFixed(2)}])`);
        ...
        residualLines.push(
          {
            v: [
              {x:d.X, y:d.y},
              {x:d.X, y:_.clamp(yHat.subset(math.index(0,i)),yExtent[0],yExtent[1])}
            ]
          }
        )
      })
  ...
  function updateSliders () {
    ...
    let theta1_value = Number((Math.max(...yExtent.map(d=>Math.abs(d)))/Math.max(...xExtent.map(d=>Math.abs(d)))).toFixed(1)),
        theta0Range = [Number(yExtent[0].toFixed(1)),Number(yExtent[1].toFixed(1))],
  ...
  let data = _.sortBy(dataRaw.data, [function(o) { return o.X; }]),
      ...
      theta2_best = dataRaw.theta2_best,
  ...
  let margin = {top: 20, right: 30, bottom: 30, left: 40},
      ...
      height = d3.select('#chart2-graph').node().getBoundingClientRect().height - margin.top - margin.bottom,
  ...
  d3.select('#chart2-best-fit').on('click', function(){
    ...
    theta2Slider.value(theta2_best);
  });
}
