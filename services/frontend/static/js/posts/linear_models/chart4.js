...
function batchGDStep (X, y, learningRate, theta) {
    let gradient = math.divide(math.multiply(math.transpose(X),math.subtract(predict(X,theta),y)),X.size()[0]);
...
function chart4 (dataRaw) {
    ...
    function updateSliders () {
        svgTheta0Slider.transition().duration(500).call(
            theta0Slider.min(dataRaw.theta0_best*.25).max(dataRaw.theta0_best*1.75).value(0)
        );
        theta0Slider.value(_.random(dataRaw.theta0_best*.25,dataRaw.theta0_best*1.75));
        svgTheta1Slider.transition().duration(500).call(
            theta1Slider.min(dataRaw.theta1_best*.25).max(dataRaw.theta1_best*1.75).value(0)
        );
        theta1Slider.value(_.random(dataRaw.theta1_best*.25,dataRaw.theta1_best*1.75));
    }
	...
	let margin = {top: 20, right: 30, bottom: 30, left: 40},
        ...
        height = d3.select('#chart4-logreg-container').node().getBoundingClientRect().height - margin.top - margin.bottom,
    ...
    let theta0Slider = d3
                    ...
                    .on('onchange.slider', val => {
                        ...
                        predComb = predRange.valueOf().map((d,i)=>({x:xRange.valueOf()[i][1],y:d[0]}))
    ...
    let svgTheta0Slider = d3
        ...
        let tooltip = d3
                    ...
                    .style("padding", "5px"),
            dot, actualY, dotX, dotY, color, bucket,
            mouseover = function(d) {
                            ...
                            color = ((actualY==0 && dotY<0.5) || (actualY==1 && dotY>=0.5)) ? 'green' : 'red'
                            if (actualY==0 && dotY<0.5) {bucket='True Negative';yPosStart=0;yNegStart=dotY}
                            else if (actualY==1 && dotY>=0.5) {bucket='True Positive';yPosStart=1;yNegStart=dotY}
                            else if (actualY==0 && dotY>=0.5) {bucket='False Positive';yPosStart=1-dotY;yNegStart=0}
                            else {bucket='False Negative';yPosStart=1-dotY;yNegStart=1}

                            tooltip.style("opacity", .9)
                            d3.select(this).style("stroke", "black").style("opacity", 1)

                            let lineData = [
                                            {id:'toPositive', color:'blue', values: [{x:dotX,y:yPosStart},{x:dotX,y:dotY}]},
                                            {id:'toNegative', color:'red', values: [{x:dotX,y:yNegStart},{x:dotX,y:1-dotY}]}
                                        ],
                                lineGenerator = d3.line().x(d=>xScale(d.x)).y(d=>yScale(d.y));
                            line2sig
                            .selectAll('.line2sig')
                            .data(lineData)
                            .join(
	...
	let timer;
    d3.select('#chart4-converge-button')
        .on("click", function() {
