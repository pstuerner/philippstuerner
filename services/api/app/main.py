...
def poly_data(degrees=30, noise=4, **kwargs):
    ...
    for degree in range(1, degrees+1):
        X_poly_train = PolynomialFeatures(degree=degree, include_bias=True).fit_transform(train['X'].to_numpy().reshape(-1,1))
        X_poly_test = PolynomialFeatures(degree=degree, include_bias=True).fit_transform(test['X'].to_numpy().reshape(-1,1))
